...
> 对应 PLAN-cn.md §1.2（Embedding 决策）与 §4（数据模型维度说明）。
...
> 注意：PLAN-cn.md §1.2 刻意排除 LangChain。老师提到的 `HuggingFaceEmbeddings`
> 只是 LangChain 对 sentence-transformers 的封装；**本项目直接用
> `sentence-transformers` 本体，不引入 LangChain**。
